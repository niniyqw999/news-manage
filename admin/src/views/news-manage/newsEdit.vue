<template>
    ...
    <el-form ref="newsFormRef" label-width="120px" class="demo-ruleForm" :model="newsForm" :rules="newsFormRules">
        <el-form-item label="标题" prop="title">
            <el-input v-model="newsForm.title" />
        </el-form-item>
        <el-form-item label="内容" prop="content">
            <Editor @event="editorChange" :content="newsForm.content" v-if="newsForm.content"></Editor>
        </el-form-item>
...
const submitForm=()=>{
    newsFormRef.value.validate((valid)=>{
        if(valid){
            //传到后端并分配到数据库
             upload('/adminapi/news/list',newsForm)
            router.back()
        }
    })
}
